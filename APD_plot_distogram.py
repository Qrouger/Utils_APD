...
def load_features(filepath):
    with open(filepath, "rb") as p:
        pre_feature_dict = pickle.load(p)
        features = pre_feature_dict.feature_dict
...
def plot_distances(filepath,list_lenght , distances, pae=None, limitA=None, limitB=None):
    print("make png")
    initial_lenght = 0
    if pae is not None:
...
def main():
    ...
    features = load_features(f"{args.in_folder}/{args.pickle_name}.pkl")
    ...
    for i, pickle_output in enumerate(pickle_list):
        ...
        if args.plot:
            lenght_list = set_lenght_list(pickle_output)
            plot_distances(f"{pickle_output}.dmap.png",lenght_list, dist, pae, limitA, limitB)
